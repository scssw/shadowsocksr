...
class MuMgr(object):
    def __init__(self):
        ...
        # 这里可以硬编码域名，或者读取文件（如你要求的）
        self.server_addr = self.get_domain_from_file()

    def get_domain_from_file(self):
        # 尝试从文件中获取服务器域名
        try:
            with open('/usr/local/shadowsocksr/myip.txt', 'r') as f:
                domain = f.read().strip()
                if domain:  # 如果文件中有内容，返回域名
                    return domain
        except Exception as e:
            pass  # 如果读取文件失败，则返回默认的域名
        return '.ssrr.today'  # 默认域名
